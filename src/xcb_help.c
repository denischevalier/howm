...
void check_other_wm(void) {
	...
	e = xcb_request_check(
	    dpy, xcb_change_window_attributes_checked(
		     dpy, screen->root, XCB_CW_EVENT_MASK, values));
	if (e != NULL) {
		xcb_disconnect(dpy);
		log_err(
		    "Couldn't register as WM. Perhaps another WM is running? "
		    "XCB returned error_code: %d",
		    e->error_code);
		exit(EXIT_FAILURE);
	}
	free(e);
 ...
 */
void move_resize(xcb_window_t win, uint16_t x, uint16_t y, uint16_t w,
		 uint16_t h) {
...
void get_atoms(const char **names, xcb_atom_t *atoms) {
	...
	for (i = 0; i < LENGTH(atoms); i++) {
		cookies[i] =
		    xcb_intern_atom(dpy, 0, strlen(names[i]), names[i]);
	...
	for (i = 0; i < LENGTH(atoms); i++) {
		...
		if (reply) {
			...
			log_warn("The atom %s has not been registered by howm.",
				 names[i]);
		}
	}
}
...
void ewmh_process_wm_state(client_t *c, xcb_atom_t a, int action) {
	if (a == ewmh->_NET_WM_STATE_FULLSCREEN) {
		...
		log_warn("Unhandled wm state <%d> with action <%d>.", a,
			 action);
	}
}
...
void setup_ewmh(void) {
	...
	if (xcb_ewmh_init_atoms_replies(ewmh, xcb_ewmh_init_atoms(dpy, ewmh),
					NULL) == 0)
...
void setup_ewmh_geom(void) {
	...
	xcb_ewmh_set_desktop_geometry(ewmh, 0, mon->rect.width,
				      mon->rect.height);
}
...
xcb_randr_output_t *randr_get_outputs(unsigned int *nr_outputs) {
	...
	if (!qer || !qer->present) return false;
	...
	*nr_outputs = xcb_randr_get_screen_resources_outputs_length(sresr);
	...
	if (!sresr || *nr_outputs < 1) goto free_sresr;
