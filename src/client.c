...
client_t *prev_client(client_t *c, workspace_t *w) {
	...
	if (!c || !w->head || !w->head->next) return NULL;
...
client_t *next_client(client_t *c) {
	if (!c || !mon->ws->head || !mon->ws->head->next) return NULL;
...
void update_focused_client(client_t *c) {
	...
	if (!mon->ws->head) {
		...
		mon->ws->prev_foc =
		    prev_client(mon->ws->c = mon->ws->prev_foc, mon->ws);
	} else if (c != mon->ws->c) {
	...
	for (c = mon->ws->head; c; c = c->next, ++all) {
		if (FFT(c)) {
			fullscreen++;
			if (!c->is_fullscreen) float_trans++;
		}
	}
	xcb_window_t windows[all];
	memset(windows, 0, sizeof(windows));

	windows[(mon->ws->c->is_floating || mon->ws->c->is_transient)
		    ? 0
		    : float_trans] = mon->ws->c->win;
	c = mon->ws->head;
	for (fullscreen += !FFT(mon->ws->c) ? 1 : 0; c; c = c->next) {
		set_border_width(c->win, c->is_fullscreen ? 0 : conf.border_px);
		xcb_change_window_attributes(
		    dpy, c->win, XCB_CW_BORDER_PIXEL,
		    (c == mon->ws->c
			 ? &conf.border_focus
			 : c == mon->ws->prev_foc ? &conf.border_prev_focus
						  : &conf.border_unfocus));
		if (c != mon->ws->c)
			windows[c->is_fullscreen
				    ? --fullscreen
				    : FFT(c) ? --float_trans : --all] = c->win;
	}
...
found:
	*temp = c->next;
	...
	if (c == w->prev_foc) w->prev_foc = prev_client(w->c, w);
	if (c == w->c || !w->head->next) {
		w->c = w->prev_foc ? w->prev_foc : w->head;
		if (m->ws == w) update_focused_client(w->c);
	}
	free(c);
	c = NULL;
	w->client_cnt--;
...
static void move_down(client_t *c) {
	...
	if (n->next == c->next)
		...
		mon->ws->head = c;
	log_info("Moved client <%p> on workspace <%d> down", c,
...
void focus_next_client(void) {
	if (!mon->ws->c || !mon->ws->head->next) return;
	log_info("Focusing next client");
	update_focused_client(mon->ws->c->next ? mon->ws->c->next
					       : mon->ws->head);
}
...
void kill_client(monitor_t *m, workspace_t *w, client_t *c) {
	...
	if (xcb_icccm_get_wm_protocols_reply(
		dpy,
		xcb_icccm_get_wm_protocols(dpy, c->win, wm_atoms[WM_PROTOCOLS]),
		&rep, NULL)) {
	...
	if (!found) xcb_kill_client(dpy, c->win);
...
void move_client(int cnt, bool up) {
	...
	if (up) {
		if (mon->ws->c == mon->ws->head) return;
		...
		if (mon->ws->c == prev_client(mon->ws->head, mon->ws)) return;
		cntcopy = cnt;
 ...
 * @ingroup commands
 */
void move_current_down(void) { move_down(mon->ws->c); }
 ...
 * @ingroup commands
 */
void move_current_up(void) { move_up(mon->ws->c); }
...
void client_to_ws(client_t *c, workspace_t *ws, bool follow) {
	...
	log_info("Moved client <%p> from <%d> to <%d>", c,
		 workspace_to_index(mon->ws), workspace_to_index(ws));
	if (follow) {
		ws->c = c;
		change_ws(ws);
	} else {
...
void draw_clients(void) {
	...
	for (c = mon->ws->head; c; c = c->next)
		if (mon->ws->layout == ZOOM && conf.zoom_gap &&
		    !c->is_floating) {
			set_border_width(c->win, 0);
			move_resize(c->win, c->rect.x + c->gap,
				    c->rect.y + c->gap,
				    c->rect.width - (2 * c->gap),
				    c->rect.height - (2 * c->gap));
		} else if (c->is_floating && !c->is_fullscreen) {
			set_border_width(c->win, conf.border_px);
			move_resize(c->win, c->rect.x, c->rect.y, c->rect.width,
				    c->rect.height);
			...
			move_resize(
			    c->win, c->rect.x + c->gap, c->rect.y + c->gap,
...
void change_client_geom(client_t *c, uint16_t x, uint16_t y, uint16_t w,
			uint16_t h) {
	log_debug(
	    "Changing geometry of client <%p> from {%d, %d, %d, %d} to {%d, "
	    "%d, %d, %d}",
...
client_t *create_client(xcb_window_t w) {
	...
	client_t *t =
	    prev_client(mon->ws->head, mon->ws); /* Get the last element. */
...
void set_fullscreen(client_t *c, bool fscr) {
	...
	if (!c || fscr == c->is_fullscreen) return;
	...
	xcb_change_property(dpy, XCB_PROP_MODE_REPLACE, c->win,
			    ewmh->_NET_WM_STATE, XCB_ATOM_ATOM, 32, fscr, data);
	if (fscr) {
		set_border_width(c->win, 0);
		change_client_geom(c, 0, 0, mon->rect.width, mon->rect.height);
		draw_clients();
	} else {
		set_border_width(c->win,
				 !mon->ws->head->next ? 0 : conf.border_px);
		arrange_windows(mon);
		draw_clients();
	}
}
...
void set_urgent(client_t *c, bool urg) {
	if (!c || urg == c->is_urgent) return;

	c->is_urgent = urg;
	xcb_change_window_attributes(
	    dpy, c->win, XCB_CW_BORDER_PIXEL,
	    urg ? &conf.border_urgent
		: c == mon->ws->c ? &conf.border_focus : &conf.border_unfocus);
}
...
void teleport_client(const int direction) {
	if (!mon->ws->c || !mon->ws->c->is_floating || mon->ws->c->is_transient)
		return;
	...
	switch (direction) {
		...
		case TOP_RIGHT:
			mon->ws->c->rect.x =
			    mon->rect.width - w - g - (2 * conf.border_px);
			mon->ws->c->rect.y = (conf.bar_bottom ? 0 : bh) + g;
			break;
		case CENTER:
			mon->ws->c->rect.x = (mon->rect.width - w) / 2;
		...
		case BOTTOM_LEFT:
			mon->ws->c->rect.x = g;
			mon->ws->c->rect.y =
			    (conf.bar_bottom ? mon->rect.height - bh
					     : mon->rect.height) -
			    h - g - (2 * conf.border_px);
			break;
		case BOTTOM_CENTER:
...
void toggle_float(void) {
	...
	if (mon->ws->c->is_floating && conf.center_floating) {
		...
		mon->ws->c->rect.y = (mon->rect.height - mon->ws->bar_height -
				      mon->ws->c->rect.height) /
				     2;
...
void resize_float_width(const int dw) {
	if (!mon->ws->c || !mon->ws->c->is_floating ||
	    (int)mon->ws->c->rect.width + dw <= 0)
		return;
	log_info("Resizing width of client <%p> from %d by %d", mon->ws->c,
...
void resize_float_height(const int dh) {
	if (!mon->ws->c || !mon->ws->c->is_floating ||
	    (int)mon->ws->c->rect.height + dh <= 0)
		return;
	log_info("Resizing height of client <%p> from %d to %d", mon->ws->c,
...
void move_float_y(const int dy) {
	if (!mon->ws->c || !mon->ws->c->is_floating) return;
	log_info("Changing y of client <%p> from %d to %d", mon->ws->c,
		 mon->ws->c->rect.y, dy);
	mon->ws->c->rect.y += dy;
	draw_clients();
}
...
void move_float_x(const int dx) {
	if (!mon->ws->c || !mon->ws->c->is_floating) return;
	log_info("Changing x of client <%p> from %d to %d", mon->ws->c,
		 mon->ws->c->rect.x, dx);
	mon->ws->c->rect.x += dx;
	draw_clients();
}
...
void make_master(void) {
	if (!mon->ws->c || !mon->ws->head->next ||
	    ...
	    !(mon->ws->layout == HSTACK || mon->ws->layout == VSTACK))
		return;
	while (mon->ws->c != mon->ws->head) move_up(mon->ws->c);
...
void focus_urgent(void) {
	...
	for (w = mon->ws_head; w != NULL; w = w->next)
		for (c = w->head; c && !c->is_urgent; c = c->next)
			;
	if (c) {
		log_info("Focusing urgent client <%p> on workspace <%d>", c,
			 workspace_to_index(w));
		change_ws(w);
...
void resize_master(const int ds) {
	 ...
	 */
	if (mon->ws->layout != HSTACK && mon->ws->layout != VSTACK) return;
	...
	if (mon->ws->master_ratio + change >= 1 ||
		...
		return;
	log_info("Resizing master_ratio from <%.2f> to <%.2f>",
		 mon->ws->master_ratio, mon->ws->master_ratio + change);
...
void toggle_bar(void) {
	...
	xcb_ewmh_geometry_t workarea[] = {
	    {0, conf.bar_bottom ? 0 : mon->ws->bar_height, mon->rect.width,
