...
int main(int argc, char *argv[]) {
	...
	while ((ch = getopt(argc, argv, "vhc:")) != -1) {
		switch (ch) {
			case 'c':
				snprintf(conf_path, sizeof(conf_path), "%s",
					 optarg);
				break;
			case 'v':
				printf("%s\n", VERSION);
	...
	if (conf_path[0] == '\0') {
		snprintf(conf_path, sizeof(conf_path), "%s/%s/%s/%s",
			 getenv("HOME"), ".config", WM_NAME, CONF_NAME);
	...
	while (running) {
		if (!xcb_flush(dpy)) log_err("Failed to flush X connection");

		FD_ZERO(&descs);
		FD_SET(dpy_fd, &descs);
		FD_SET(sock_fd, &descs);

		if (select(MAX_FD(dpy_fd, sock_fd), &descs, NULL, NULL, NULL) >
		    0) {
			if (FD_ISSET(sock_fd, &descs)) {
				...
				if (n > 0) {
					...
					if (write(cmd_fd, &ret, sizeof(int)) ==
					    -1)
						log_err(
...
void howm_info(void) {
	...
	for (ws = mon->ws_head; ws != NULL; ws = ws->next) {
		fprintf(stdout, "%d:%u:%d:%u:%u\n", ws->layout,
			...
			monitor_to_index(mon));
	}
	fflush(stdout);
#else
	fprintf(stdout, "%d:%d:%d:%u:%u\n", mon->ws->layout,
		workspace_to_index(mon->ws), cur_state, mon->ws->client_cnt,
		monitor_to_index(mon));
	fflush(stdout);
#endif
}
...
uint32_t get_colour(char *colour) {
	...
	g = ((rgb >> 8) & 0xFF) * 257;
	b = (rgb & 0xFF) * 257;
	rep = xcb_alloc_color_reply(dpy, xcb_alloc_color(dpy, map, r, g, b),
				    NULL);
	if (!rep) {
